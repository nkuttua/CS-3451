...
public class Triangle extends Shape {
  ...
  public Hit intersectRay(Ray ray) 
  {
    ...
    // ===== (TODO) Stage 1 begins =======    
    PVector intersection = PVector.add(ray.origin, PVector.mult(ray.direction, t)); // (TODO 1-1) Compute the intersection point using the ray class and the computed t value.
    ...
    PVector bary = new PVector();
    
    PVector a = PVector.sub(v2, v1);
    PVector b = PVector.sub(v3, v1);
    PVector c = PVector.sub(intersection, v1);
    
    float aa = PVector.dot(a, a);
    float ab = PVector.dot(a, b);
    float bb = PVector.dot(b, b);
    float ca = PVector.dot(c, a);
    float cb = PVector.dot(c, b);
    
    float denom = (aa * bb - ab * ab);

    bary.x = (bb * ca - ab * cb) / denom;
    bary.y = (aa * cb - ab * ca) / denom;
    bary.z = 1 - bary.x - bary.y;

    // Stage 1 Last Part (possible issue may be having to flip the normal)
    if (Debug.debug)
    {
    ...
    if ((bary.x > 0) && (bary.y > 0) && (bary.z > 0)) {
       hit = true; 
    } else if ((bary.x < 0) && (bary.y < 0) && (bary.z < 0)) {
       hit = true; 
    } else {
       hit = false;
    }  
    ...
    if (PVector.dot(this.normal, ray.direction) >= 0) {
        this.normal = PVector.mult(this.normal, -1);
    }
