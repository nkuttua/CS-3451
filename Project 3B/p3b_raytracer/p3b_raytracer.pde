...
PVector shadeHelper(Hit sceneHit, int depth) {
  ...
  if (Debug.DO_REFLECTION && depth > 0 && sceneHit.hitShape.mat.kRefl > 0)
  {
    ...
    float NdotD = PVector.dot(N, PVector.mult(d, -1));
    PVector R = PVector.add(d, PVector.mult(N, 2 * NdotD)).normalize();
  ...
  for (Light currLight: lights) {
    if (Debug.DO_SHADOWS)
    {
      ...
      PVector shadow_ray_dir = PVector.sub(currLight.pos, sceneHit.interPoint).normalize();
      Ray shadow_ray = new Ray(shadow_ray_orig, shadow_ray_dir); // Create the shadow ray
      // ===== (TODO) Stage 3 ends =======
    ...
    PVector diffuseContribution = PVector.mult(
      pairwiseMult(currLight.col, currShapeDifCol),
      diffuseFactor
    );
    ...
    if (Debug.DO_SPECULAR)
    {
      ...
      PVector H = PVector.sub(L, D).normalize(); // H is the halfway vector that is normalized
      float p = sceneHit.hitShape.mat.specPow;  // Adjust the sharpness of the specular
      float specular_coeff = pow(Math.max(0, PVector.dot(H, N)), p); // compute the spectacular coefficient, using H, N, and p
      // ===== (TODO) Stage 2b ends =======
