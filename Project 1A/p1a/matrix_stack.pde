...
void gtPushMatrix() {
  ...
  // (TODO)
  Matrix temp = mat_copy(mat_stack.get(mat_stack.size() - 1));
  mat_stack.add(temp);
}
...
void updateTopMatrix(Matrix m) {
  ...
  ctm = mat_multiply(ctm, m);
  mat_stack.set(mat_stack.size() - 1, ctm);
}
...
void gtScale(float x, float y, float z) {
  ...
  scaleMat.values[0][0] = x;
  scaleMat.values[1][1] = y;
  scaleMat.values[2][2] = z;
  updateTopMatrix(scaleMat);
}
...
void gtRotateX(float theta) {
  ...
  // (TODO) 
  Matrix trans = new Matrix();
  trans.values[1][1] = cos(radians(theta));
  trans.values[1][2] = -sin(radians(theta));
  trans.values[2][1] = sin(radians(theta));
  trans.values[2][2] = cos(radians(theta));
  updateTopMatrix(trans);
}
...
void gtRotateY(float theta) {
  ...
  // (TODO)
  Matrix trans = new Matrix();
  trans.values[0][0] = cos(radians(theta));
  trans.values[0][2] = sin(radians(theta));
  trans.values[2][0] = -sin(radians(theta));
  trans.values[2][2] = cos(radians(theta));
  updateTopMatrix(trans);
}
...
void gtRotateZ(float theta) {
  ...
  Matrix trans = new Matrix();
  trans.values[0][0] = cos(radians(theta));
  trans.values[0][1] = -sin(radians(theta));
  trans.values[1][0] = sin(radians(theta));
  trans.values[1][1] = cos(radians(theta));
  updateTopMatrix(trans);
}
