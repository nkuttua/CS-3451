...
void render_scene() {
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      //PVector dirVec = new PVector(U, -V, -d).normalize(); // works for images 1, 2, 4
      
      //PVector dirVec = new PVector(-d, -V, -U).normalize(); // works for image 3
      
      
      PVector temp1 = PVector.mult(uvw[0], U);
      PVector temp2 = PVector.mult(uvw[1], -V);
      PVector temp3 = PVector.mult(uvw[2], -d);
      PVector dirVec = PVector.add(temp1, temp2);
      dirVec = PVector.add(dirVec, temp3).normalize();
...
Hit rayIntersectScene(Ray ray) {
  ...
  for (int i = 0; i < shapes.size(); i++) {
    Hit curr = shapes.get(i).intersectRay(ray);
    if (curr != null && curr.t < minT) {
      minT = curr.t;
      minHit = curr;
    }
  }
...
PVector shadeHelper(Hit sceneHit) {
  if (sceneHit == null) {
    return bgColor;  // Return background color if there is no hit
  }
  PVector totalCol = new PVector(0, 0, 0);  // Initialize the total color to black
  ...
  for (Light currLight: lights) {
    ...
    PVector L = PVector.sub(currLight.pos, sceneHit.interPoint).normalize();
    ...
    float NdotL = Math.max(0, PVector.dot(N, L));
    
    float tempX = NdotL * currShapeDifCol.x * currLight.col.x;
    float tempY = NdotL * currShapeDifCol.y * currLight.col.y;
    float tempZ = NdotL * currShapeDifCol.z * currLight.col.z;
    
    PVector temp = new PVector(tempX, tempY, tempZ);
    totalCol.add(temp);
    ...
    if (debug_flag) {
      ...
      println("light's contribution (light color * diffuse color * (N dot L)): " + totalCol);
      println();
    }
    // ===== (TODO) Stage 4 ends =======
  }
